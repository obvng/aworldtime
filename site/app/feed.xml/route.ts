...
import { SITE_ORIGIN, xmlEscape } from "@/lib/seo";

export async function GET() {
  const posts = await listPublishedPosts();
  const items = posts.map((post) => `<item><title>${xmlEscape(post.title)}</title><link>${SITE_ORIGIN}/blog/${post.slug}</link><guid>${SITE_ORIGIN}/blog/${post.slug}</guid><description>${xmlEscape(post.excerpt)}</description><pubDate>${new Date(post.published_at as string).toUTCString()}</pubDate></item>`).join("");
